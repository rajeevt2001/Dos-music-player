...
// NOTE: Turbo C++ does not natively support 64-bit integers. 
// We define them as 'long' just to try to bypass the compiler syntax errors.
...
static int have_simd(void)
{
    ...
    if (CPUInfo[0] > 0)
    {
        ...
        g_have_simd = (CPUInfo[3] & (1UL << 26)) + 1; /* SSE2 */
    }
end:
...
#define DQ(x) 9.53674316e-07f/x, 7.56931807e-07f/x, 6.00777173e-07f/x
    ...
    for (i = 0; i < bands; i++)
    {
        ...
        for (m = 4; m; m >>= 1)
        {
            ...
            {
                int b = get_bits(bs, 6);
                s = g_deq_L12[ba*3 - 6 + b % 3]*(1UL << 21 >> b/3);
            }
            *scf++ = s;
        }
    }
    ...
    for (j = 0; j < 4; j++)
    {
        ...
        for (i = 0; i < 2*sci->total_bands; i++)
        {
            ...
            if (ba != 0)
            {
                ...
                {
                    unsigned mod = (2UL << (ba - 17)) + 1;    /* 3, 5, 9 */
...
static void L3_read_scalefactors(uint8_t *scf, uint8_t *ist_pos, const uint8_t *scf_size, const uint8_t *scf_count, bs_t *bitbuf, int scfsi)
{
    ...
    for (i = 0; i < 4 && scf_count[i]; i++, scfsi *= 2)
    {
        ...
        } else
        {
            ...
            {
                int max_scf = (scfsi < 0) ? (1UL << bits) - 1 : -1;
                for (k = 0; k < cnt; k++)
                {
...
static void L3_decode_scalefactors(const uint8_t *hdr, uint8_t *ist_pos, bs_t *bs, const L3_gr_info_t *gr, float *scf, int ch)
{
    ...
    gain = L3_ldexp_q2(1UL << (MAX_SCFI/4),  MAX_SCFI - gain_exp);
...
#define BSPOS         ((bs_next_ptr - bs->buf)*8 - 24 + bs_sh)
    ...
    uint32_t bs_cache = (((bs_next_ptr[0]*256UL + bs_next_ptr[1])*256UL + bs_next_ptr[2])*256UL + bs_next_ptr[3]) << (bs->pos & 7);
...
#define DEQ_COUNT1(s) if (leaf & (128UL >>s)) { dst[s] = ((int32_t)bs_cache < 0) ? -one : one; FLUSH_BITS(1) }
        RELOAD_SCALEFACTOR;
        DEQ_COUNT1(0);
        DEQ_COUNT1(1);
        RELOAD_SCALEFACTOR;
...
static void L3_stereo_process(float *left, const uint8_t *ist_pos, const uint8_t *sfb, const uint8_t *hdr, int max_band[3], int mpeg2_sh)
{
    ...
    for (i = 0; sfb[i]; i++)
    {
        ...
        if ((int)i > max_band[i % 3] && ipos < max_pos)
        {
            ...
            {
                kl = 1;
                kr = L3_ldexp_q2(1, (ipos + 1) >> 1UL << mpeg2_sh);
                if (ipos & 1)
                {
                    kl = kr;
                    kr = 1;
